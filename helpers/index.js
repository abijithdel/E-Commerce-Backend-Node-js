...
function CartItemCount(user_id){
    return new Promise( async (resolve, reject) => {
        try {
            ...
            if(!Cart){
                return resolve({status:false,message:'Not Exist The User Cart',count:0})
            }
            const leng = Cart.product_ids.length
            resolve({status:true,message:'Successfully Fetch User Cart',count:leng})
        } catch (error) {
            console.log(error)
            reject({status:false,message:error.message})
        }
...
module.exports = { OneProduct, AddtoCart, CartItemCount, CartItems, SaveAddress, GetAddress}